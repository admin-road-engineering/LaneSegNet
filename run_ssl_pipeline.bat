...
echo =======================================================
echo  LaneSegNet - SSL Pre-training Pipeline (Data Ready)
echo =======================================================
echo This script will execute the advanced pipeline:
echo 1. [SKIP] Data collection (already complete: 1,100 images)
echo 2. Run Self-Supervised Pre-training (MAE)
echo 3. Fine-tune the final model with pre-trained encoder and OHEM loss
echo.
echo Configuration:
echo   SSL Epochs: %SSL_EPOCHS%, Batch Size: %SSL_BATCH_SIZE%
echo   Fine-tune Epochs: %FINETUNE_EPOCHS%, Batch Size: %FINETUNE_BATCH_SIZE%
echo.
echo Press any key to begin SSL pre-training...
pause
echo.
...
rem == STEP 1: VERIFY DATA AVAILABILITY ==
echo.
echo =======================================================
echo  STEP 1: Verifying Data Availability
echo =======================================================
python check_ssl_readiness.py
if %errorlevel% neq 0 (
    echo [ERROR] Data verification failed. Please check data availability.
    pause
    exit /b 1
)
echo [SUCCESS] Data verification complete - ready for SSL pre-training.
echo.
...
echo =======================================================
echo [SUCCESS] Data Verification: Complete (1,100 images)
